...
void init_opengl(void)
{
	...
	//initialize fonts for text in the rectangles
	initialize_fonts();
}
...
void render()
{
	...
	for(int i = 0; i < 5; i++)
	{
		s = &g.box[i]; //5 boxes passed and rendered
		glPushMatrix();
	...
	r.bot = 428;
	r.left = 150;
	ggprint16(&r, 16, 0x00ffff0, "Design"); 

	r.bot -= 45;
	r.left += 70;
	ggprint16(&r, 16, 0x00ffff0, "Coding"); 

	r.bot -= 45;
	r.left += 50;
	ggprint16(&r, 16, 0x00ffff0, "Testing");

	r.bot -= 40;
	r.left +=70;
	ggprint16(&r, 16, 0x00ffff0, "Maintenance");
